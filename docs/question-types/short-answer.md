...
demand an exact answer with backticks.
...
[^5]: `"fold"` is the default and strips diacritics; `"keep"` preserves them,
so an inexact `Brasília` no longer accepts `Brasilia`. It applies only to
literals compared inexactly -- a backtick-enclosed answer is already verbatim,
and a regex answers to its own `n` flag instead. Case folding and whitespace
normalization are unaffected either way.
